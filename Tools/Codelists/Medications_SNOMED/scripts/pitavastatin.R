...
##################################################################################
# Extraction - rule 1: has a relationship specifying an ingredient and destination term contains "pitavastatin" or "pitavastatin"
...
pitavastatin.dest.list <- human_read_rels %>%
  filter(
    str_detect(destination.term, "pitavastatin|pitavastatin"))%>%
  select(destinationId)%>%
  distinct(destinationId)%>%
  .[[1]]
# all terms used to depict a relationship referring to pitavastatin 

## apply rule 
pitavastatin.codelist<- relationships_active %>%
  filter(
    typeId %in% ingredient.rel.list & 
    destinationId %in% pitavastatin.dest.list) %>% 
  distinct(sourceId)%>% 
  left_join(FSN, by=c("sourceId"="conceptId"))%>%
  rename(conceptId=sourceId)

View(pitavastatin.codelist)

write.csv(pitavastatin.codelist, "codelists/pitavastatin_dmd.csv",row.names=F)
